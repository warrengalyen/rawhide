...
impl<'a> NefDecoder<'a> {
  pub fn new(buf: &'a [u8], tiff: TiffIFD<'a>, rawhide: &'a RawHide) -> NefDecoder<'a> {
    NefDecoder {
      buffer: buf,
      tiff: tiff,
      rawhide: rawhide,
  ...
  fn image(&self) -> Result<Image,String> {
    ...
    let image = if camera.model == "NIKON D100" {
      ...
      if compression == 1 || size == width*height*bps/8 {
        ...
        cpp = 3;
        self.decode_snef_compressed(src, width, height)?
      } else if compression == 34713 {
        self.decode_compressed(src, width, height, bps)?
      } else {
        return Err(format!("NEF: Don't know compression {}", compression).to_string())
      }
    };

    let mut img = Image::new(camera, width, height, self.get_wb()?, image);
    if cpp == 3 {
      img.cpp = 3;
