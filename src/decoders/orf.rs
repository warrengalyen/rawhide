...
impl<'a> OrfDecoder<'a> {
  pub fn new(buf: &'a [u8], tiff: TiffIFD<'a>, rawhide: &'a RawHide) -> OrfDecoder<'a> {
    OrfDecoder {
      buffer: buf,
      tiff: tiff,
      rawhide: rawhide,
...
impl<'a> Decoder for OrfDecoder<'a> {
  fn image(&self) -> Result<RawImage,String> {
    ...
    let camera = if width >= camera.highres_width {
      self.rawhide.check_supported_with_mode(&self.tiff, "highres")?
    } else {
      camera
    };
...
impl<'a> OrfDecoder<'a> {
  ...
  fn get_wb(&self) -> Result<[f32;4],String> {
    ...
    if redmul.is_some() && bluemul.is_some() {
      ...
      let ifd = TiffIFD::new(self.buffer, off, 0, 0, 0, self.tiff.get_endian())?;
