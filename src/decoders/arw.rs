...
impl<'a> ArwDecoder<'a> {
    fn decode_compressed(&self, camera: &Camera, raw: &TiffIFD) -> Result<Image, String> {
        ...
        let count = fetch_tag!(raw, Tag::StripByteCounts, "ARW: Couldn't find byte count")
            .get_u32(0) as usize;
    ...
    fn get_wb(&self) -> Result<[f32; 4], String> {
        let priv_offset = fetch_tag!(
            self.tiff,
            Tag::DNGPrivateArea,
            "ARW: Couldn't find private offset"
        )
        .get_u32(0);
        let priv_tiff = TiffIFD::new(self.buffer, priv_offset as usize, 0, LITTLE_ENDIAN);
        let sony_offset = fetch_tag!(priv_tiff, Tag::SonyOffset, "ARW: Couldn't find sony offset")
            .get_u32(0) as usize;
        let sony_length = fetch_tag!(priv_tiff, Tag::SonyLength, "ARW: Couldn't find sony length")
            .get_u32(0) as usize;
        let sony_key =
            fetch_tag!(priv_tiff, Tag::SonyKey, "ARW: Couldn't find sony key").get_u32(0);
        ...
        if grgb_levels.is_some() {
            ...
            Ok([
                levels.get_u16(1) as f32,
                levels.get_u16(0) as f32,
                levels.get_u16(2) as f32,
                f32::NAN,
            ])
