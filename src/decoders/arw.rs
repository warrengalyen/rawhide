...
impl<'a> ArwDecoder<'a> {
    ...
    fn get_wb(&self) -> Result<[f32; 4], String> {
        ...
        let sony_offset = fetch_tag!(
            priv_tiff,
            Tag::SONY_OFFSET,
            "ARW: Couldn't find sony offset"
        )
        .get_u32(0) as usize;
        let sony_length = fetch_tag!(
            priv_tiff,
            Tag::SONY_LENGTH,
            "ARW: Couldn't find sony length"
        )
        .get_u32(0) as usize;
        let sony_key =
            fetch_tag!(priv_tiff, Tag::SONY_KEY, "ARW: Couldn't find sony key").get_u32(0);
