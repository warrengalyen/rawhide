...
impl RawHide {
  ...
  pub fn check_supported_with_everything<'a>(
    &'a self,
    make: &str,
    model: &str,
    mode: &str,
  ...
  pub fn check_supported_with_mode<'a>(
    &'a self,
    tiff: &'a TiffIFD,
    mode: &str,
  ) -> Result<&Camera, String> {
  ...
  pub fn decode_safe(&self, path: &str) -> Result<RawImage, String> {
    match panic::catch_unwind(|| {
      ...
      Ok(val) => val,
      Err(_) => {
        Err("Caught a panic while decoding, please file a bug and attach a sample file".to_string())
      }
    }
  }
}
