...
impl<'a> TfrDecoder<'a> {
  pub fn new(buf: &'a [u8], tiff: TiffIFD<'a>, rawhide: &'a RawHide) -> TfrDecoder<'a> {
    TfrDecoder {
      buffer: buf,
      tiff: tiff,
      rawhide: rawhide,
...
impl<'a> Decoder for TfrDecoder<'a> {
  fn image(&self) -> Result<Image,String> {
    ...
    let image = self.decode_compressed(src, width, height)?;
...
impl<'a> TfrDecoder<'a> {
  ...
  fn decode_compressed(&self, src: &[u8], width: usize, height: usize) -> Result<Vec<u16>,String> {
    ...
    let decompressor = LjpegDecompressor::new(src, false)?;
